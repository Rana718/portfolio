...
It does **not** promise snapshot iteration, immediate physical deletion, wait-free writes, or reclamation of old table shells after a resize.
...
The `#[repr(align(128))]` shard alignment and `CachePadded<AtomicUsize>` reduce false sharing. Threads modifying the length counter of shard 0 should not constantly invalidate the cache line containing shard 1's hot metadata.
...
```mermaid
flowchart TD
    START["Insert key"] --> FIND{"Key already present?"}
    FIND -->|yes| REPLACE["Atomic value replacement"]
    FIND -->|no| LIMIT{"At 70% threshold?"}
...
Growing the table is the _only_ place we use a mutex, and only to serialise concurrent resize attempts — readers are never blocked:
...
Readers loading `self.table` with `Acquire` will see the new table pointer after publication. Readers that loaded the previous pointer just before the swap can safely finish against the old table because its entries remain stable.

That last limitation deserves precise wording: **the current code does not retire old `SlotTable` shells through EBR**. After publishing the new table, old readers may still hold the old table pointer, so it cannot be freed immediately. The implementation currently leaves those old shells allocated. Values remain valid because both tables point to stable entries, but repeated growth trades memory for simple reader safety.
...
sequenceDiagram
    ...
    W->>W: lock grow mutex
    W->>W: allocate 2x table and rehash live entries
    W->>S: Release-store new table pointer
    R->>R: finish probing old table safely
    Note over S,W: old table shell is currently retained
```

Readers never take `grow_lock`; the mutex only prevents multiple writers from rebuilding the same shard simultaneously. A writer that waited for the lock rechecks the threshold because another writer may already have completed the growth.
...
2. Published entry shells are not freed while concurrent operations may find them.
...
7. The load threshold leaves at least one null slot, so an unsuccessful probe terminates.

Any future optimization—SIMD probing, entry reclamation, slot reuse, or table-shell reclamation—must preserve all seven.
...
| `insert` new key      | length CAS + slot CAS + probing     | O(1) amortized      |
| `update`              | CAS loop; closure may retry         | O(1) amortized      |
| `remove`              | atomic null swap + EBR retirement   | O(1) amortized      |
| `grow`                | one shard's writer mutex            | O(entries in shard) |
...
`for_each`, `keys`, `retain`, and `clear` walk shard tables while pinned. They do not stop writers and do not create a global snapshot. An iteration may see an update in one shard and miss a later update in another. That is acceptable for diagnostics, maintenance passes, and eventually consistent views, but not for transactions or exact point-in-time exports.

`retain` and `clear` use the same logical-deletion mechanism as `remove`: each live value is atomically swapped to null and retired. Entry shells remain in their probe positions.
...
- **Old table reclamation** — retire previous `SlotTable` shells through EBR after resize.
- **Participant lifecycle** — unregister exited threads or transfer their pending garbage through the orphan path.
- **Snapshot iteration** — add versioning or an explicit snapshot abstraction when exact views are required.
